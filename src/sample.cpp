...
// [[Rcpp::export]]
NumericMatrix Rcpp__rexmo_markovian_acdp(const std::size_t n,
  ...
  for (auto k = R_xlen_t{0}; k < n; ++k) {
    ...
    dt2adcp(values.cbegin(), values.cend(), times.cbegin(), times.cend(),
            out(k, _).begin());
  }
...
// [[Rcpp::export]]
NumericMatrix Rcpp__rcamo_esm_adcp(const std::size_t n,
                                   const NumericVector &times,
                                   const std::size_t d, const double alpha,
                                   const double beta) {
  using exponential_distribution =
      rmolib::random::exponential_distribution<double>;

...
test_that("`FrankH2ExtArch3FParam`-class is correctly initialized", {
  ...
  expect_equal(parm, FrankH2ExtArch3FParam(composition, lambda, nu))
  expect_equal(parm, FrankH2ExtArch3FParam(composition, lambda, rho = rho))
  expect_equal(parm, FrankH2ExtArch3FParam(composition, lambda, tau = tau))
})

test_that("`FrankH2ExtArch3FParam`-class setters can be used in arbitrary order", { # nolint
  parm <- FrankH2ExtArch3FParam(composition, lambda, nu, fraction)

  parm2 <- FrankH2ExtArch3FParam()

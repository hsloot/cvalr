...
test_that("`ClaytonH2ExtArch3FParam`-class is correctly initialized", {
  ...
  expect_equal(parm, ClaytonH2ExtArch3FParam(partition, lambda, nu))
  expect_equal(parm, ClaytonH2ExtArch3FParam(partition, lambda, rho = rho))
  expect_equal(parm, ClaytonH2ExtArch3FParam(partition, lambda, tau = tau))
})

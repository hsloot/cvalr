...
models <- c(
  list(AlphaStableExtMO2FParam(d, lambda, alpha = alpha[[1]] / fraction)),
  models)
nu <- purrr::map_dbl(models[1:2], getNu)

test_that("`AlphaStableH2ExtMO3FParam`-class is correctly initialized", {
  parm <- AlphaStableH2ExtMO3FParam()
  expect_s4_class(parm, "AlphaStableH2ExtMO3FParam")
  ...
  expect_equal(parm, AlphaStableH2ExtMO3FParam(partition, lambda, nu, fraction))
  expect_equal(parm, AlphaStableH2ExtMO3FParam(partition, lambda, fraction = fraction, rho = rho))
  expect_equal(parm, AlphaStableH2ExtMO3FParam(partition, lambda, fraction = fraction, tau = tau))
